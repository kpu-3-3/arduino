...
#define RED1 4 //적외선센서 //1일때가 약물 없음
#define RED2 7
#define RED3 8
#define RED4 11
...
void time_pill(){ //안드로이드에 데이터 전송
    ...
    if(cur-prev==10){ //안드로이드에 데이터 전송 
      ...
      if(avr_RED1>=0.9) {Serial.println("1번 미복용");bt.write("18");setNeopixel();}//미복용상태전송
      else {
        Serial.println("1번 복용"); 
        if(set_1==0){
           set_1=1; //메세지 전송여부확인
           bt.write("10");//복용 상태 전송
           }
        } 
        
      
      if(avr_RED2>=0.9) {Serial.println("2번 미복용");bt.write("28");setNeopixel();}
      else {
      ...
      if(avr_RED3>=0.9) {Serial.println("3번 미복용");bt.write("38");setNeopixel();}
      else {
      ...
      if(avr_RED4>=0.9) {Serial.println("4번 미복용");bt.write("48");setNeopixel();}
      else {
